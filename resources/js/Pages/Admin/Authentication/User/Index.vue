...
import ActionForm from '@/Pages/Admin/Authentication/User/Partials/ActionForm.vue';
...
const search = () => {
  form.get(route('admin-user.index', { search: form.search }), {
    replace: true,
    preserveState: true,
    preserveScroll: true,
    only: ['filters', 'users'],
          ...
          <div class="bg-white border border-gray-200 rounded-md shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200">
              ...
              <div class="sm:col-span-2 md:grow">
                <div class="flex justify-end gap-x-2">
                  <PrimaryButtonLink :href="route('admin-user.create')">Create</PrimaryButtonLink>
                </div>
              </div>
            </div>
            <!-- End Header -->
            ...
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-200">
                <tr v-for="each in props.users.data" :key="each.id">
                ...
                <tr v-if="props.users.data.length === 0">
                  <td class="size-px whitespace-nowrap text-center" colspan="4">
                    <div class="px-6 py-3">
            ...
            <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200">
              <div>
                <span class="text-sm font-normal text-gray-700 block w-full md:inline md:w-auto">
                  Showing
                  <span v-if="props.users.total === 0">{{ props.users.total }}</span>
                  <span v-else-if="props.users.total === 1">{{ props.users.from }}</span>
                  <span v-else>{{ props.users.from + ' - ' + props.users.to }}</span>
                  of
                  <span class="font-semibold text-gray-900">{{ props.users.total }}</span>
                </span>
              </div>
              <div>
